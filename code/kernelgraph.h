...
public:
    ...
    virtual void init_build() = 0;
    virtual void init_build_v2() = 0;
    virtual void fullybuild() = 0;
    virtual void fullybuild_v2() = 0;
...
public:
    ...
    void cal_neighbors(int const& a) {
        std::priority_queue<std::pair<dist_t,idx_t>> topk;
        for (int b = a*10000+data->max_vertices()/10000; b <a*10000+1000; b++) {
            dist_t now_dis = data->l2_distance_idx(a, b);
            if (topk.size() >= degree*10 && topk.top().first<now_dis) continue;
            if (topk.size() >= degree*10 && topk.top().first>now_dis) topk.pop();
            topk.push(std::make_pair(now_dis, b));
        }
        auto offset = a << vertex_offset_shift;
        int len = degree < topk.size() ? degree : topk.size();
        edges[offset] = len;
        for(int i = len-1;i >= 0; --i){
            edges[offset + i + 1] = topk.top().second;
            topk.pop();
        }
        rank_edges(offset);
    }
    ...
    void init_build() {
        ...
        for (int a = 0; a < data->max_vertices()/10000; a++) {
            threads.push_back(std::thread(&KernelFixedDegreeGraph::cal_neighbors, this, a));
        }
    ...
    void cal_neighbors_v2(int const& a) {
        std::priority_queue<std::pair<dist_t,idx_t>> topk;
        for (int b = a+1; b <data->max_vertices()&&b<a+10000; b++) {
            dist_t now_dis = data->l2_distance_idx(a, b);
            if (topk.size() >= degree*10 && topk.top().first<now_dis) continue;
            if (topk.size() >= degree*10 && topk.top().first>now_dis) topk.pop();
            topk.push(std::make_pair(now_dis, b));
    ...
    void init_build_v2() {
        // #pragma omp parallel for
        std::vector<std::thread> threads;
        for (int a = 0; a < data->max_vertices(); a+=10000) {
            threads.push_back(std::thread(&KernelFixedDegreeGraph::cal_neighbors_v2, this, a));
        }
        for (std::thread & t: threads) {
            t.join();
        }
     ...
     void build_by_group(int const& t) {
        for (int a = 1+10000*t; a < 10000*(t+1)&&a<data->max_vertices(); a++) {
            ...
            if(!visited.count(t)) {
                visited.insert(t);
                q.push(std::make_pair(data->l2_distance_idx(t,a),t));
            }
            std::priority_queue<std::pair<dist_t,idx_t>> topk;
            ...
            for(int iter = 0;iter < max_step && !q.empty();++iter){
                ...
                for(int i = 0;i < d;++i){
                    ...
                    if(visited.count(start))
                        continue;
                    q.push(std::make_pair(data->l2_distance_idx(start,a),start));
            ...
            ++total_explore_times;
            int num_neighbors = degree < topk.size() ? degree : topk.size();
            auto offset = a << vertex_offset_shift;
            edges[offset] = num_neighbors;
            // TODO:
            // it is possible to save this space --- edges[offset]
            ...
            for(int i = num_neighbors-1;i >= 0;--i){
                // std::cout<<topk.top().second<<std::endl;
                edges[offset + i + 1] = topk.top().second; 
                add_edge(topk.top().second, a);
                topk.pop();
            }
            rank_edges(offset);
        }
    ...
    void fullybuild_v2() {
        std::vector<std::thread> threads;
        const int num_start_point = data->max_vertices()/10000;
        for (int t = 0; t <= num_start_point; t++) {
            threads.push_back(std::thread(&KernelFixedDegreeGraph::build_by_group, this, t));
        }
        for (std::thread & t: threads) {
            t.join();
        }
    ...
    void fullybuild() {
        const int num_start_point = data->max_vertices()/10000;
        for (int a = num_start_point; a < data->max_vertices(); a++) {
            std::priority_queue<std::pair<dist_t,idx_t>,std::vector<std::pair<dist_t,idx_t>>,std::greater<std::pair<dist_t,idx_t>>> q;
            std::unordered_set<idx_t> visited;
            for(int b = 0;b < num_start_point;++b){
                if(visited.count(b))
                    continue;
                visited.insert(b);
                q.push(std::make_pair(data->l2_distance_idx(b,a),b));
            }
            std::priority_queue<std::pair<dist_t,idx_t>> topk;
            const int max_step = 1000000;
            dist_t min_dist = 1e100;
